...
//    FILE: AS56000.cpp
//  AUTHOR: Rob Tillaart
//MODIFIED BY: Mathias L. C.
// VERSION: 0.4.0macco
// PURPOSE: Arduino library for AS5600 magnetic rotation meter
//    DATE: 2022-05-28
//     URL: https://github.com/RobTillaart/AS5600 (Original)
...
bool AS5600::SetOffsetPos(EncoderUnits _offsetPosition){
  this->_offsetPos = _offsetPosition;
}
EncoderUnits AS5600::GetOffsetPosition(){
  return this->_offsetPos;
}

EncoderUnits AS5600::getCumulativePosition() {
  int16_t value = readReg2(AS5600_ANGLE) & 0x0FFF;
  //  whole rotation CW?
  //  less than half a circle
  if ((_lastRead > 2048) && ((_lastRead - 2048) > value))  // cuando last_read > mediaVuelta &&
  {
    _position = _position + (4096 - (_lastRead + value));
  }
  //  whole rotation CCW?
  //  less than half a circle
  else if ((value > 2048) && (_lastRead < (value - 2048))) {
    _position = _position - 4096 - _lastRead + value;
  } else {
    _position = _position - _lastRead + value;
  }
  ...
  return _position + _offsetPos;
}


int32_t AS5600::getRevolutions() {
  int32_t p = _position.getLaps();  //  divide by 4096
  return p;
  // if (p < 0) p++;
  // return p;
}
...
EncoderUnits AS5600::resetPosition(EncoderUnits position) {
  EncoderUnits old = _position;
  _position = position;
  return old;
}


EncoderUnits AS5600::resetCumulativePosition(EncoderUnits position) {
  /*_lastRead = readReg2(AS5600_RAW_ANGLE) & 0x0FFF;
  EncoderUnits old = _position;
  _position = position;
  return old;
  */
