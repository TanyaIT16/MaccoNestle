...
void pingAll(bool searchCommand) {
    if (searchCommand) {
        RGBcolor(0, 255, 64);
        searchNum = EEPROM.read(0); // Leer el número de servos guardados

        if (searchNum == 255 || searchNum == 0) { // Si no hay datos previos en EEPROM, realizar la búsqueda
            searchNum = 0;
            searchedStatus = true;
            ...
            for (int i = 0; i <= MAX_ID; i++) {
                ...
                if (PingStatus != -1) {
                    ...
                    EEPROM.write(0, searchNum + 1); // Guardar el número total de servos detectados
                    EEPROM.write(1 + searchNum, i); // Guardar el ID del servo en EEPROM
                    EEPROM.commit();
            ...
            Serial.println("ID:");
            for (int j = 0; j < searchNum; j++) {
                Serial.print(listID[j]); Serial.print(" ");
                Serial.println("");
            }

            Serial.println("Type:");
            for (int j = 0; j < searchNum; j++) {
                Serial.print(ServoType[j]); Serial.print(" ");
                Serial.println("");
            }
            ...
            Serial.println("Usando datos de EEPROM...");
            searchNum = EEPROM.read(0); // Recuperar número de servos guardados
            
            for (int j = 0; j < searchNum; j++) {
                listID[j] = EEPROM.read(1 + j);
                Serial.print("Servo encontrado en EEPROM: ID ");
                Serial.println(listID[j]);
            }
        }
