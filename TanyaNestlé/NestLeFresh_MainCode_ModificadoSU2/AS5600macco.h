...
#define AS5600_LIB_VERSION (F("0.4.0Macco"))
...
public:
  ...
  EncoderUnits getCumulativePosition();
  ...
  EncoderUnits resetPosition(EncoderUnits position = EncoderUnits());
  //  resets position and internal lastPosition
  //  returns last position.
  EncoderUnits resetCumulativePosition(EncoderUnits position = EncoderUnits());

  bool SetOffsetPos(EncoderUnits _offsetPosition);
  EncoderUnits GetOffsetPosition();
...
protected:
  ...
  //EncoderUnits positionCumulative;
  EncoderUnits _position;
  EncoderUnits _offsetPos;
  long _lastRead = 0;
};
